...
class SSHer(object):
	...
	def __init__(self,host,user,passwd):
		...
		try: self.conn.connect(host,username=user,password=passwd)
		except Exception as e: raise RaiseRuntimeError("connection failed")


	def return_code(self, cmd):
...
def ssh_conn_config(host, user, passwd, lv_sasl_login='', lv_sasl_passwd=''):
	HYP_CONFIG_RPM = "webvirtmgr-hyp-1.6-1.el6.x86_64"

	import socket

	siteDomain = socket.gethostname()

	try: ssh_conn = SSHer(host,user,passwd)
	except Exception as e: return {'error': e.message}

	result = ssh_conn.cmd_output("ls /etc/ | grep 'redhat-release'").strip()

	if result == "":
		ssh_conn.close()
		return {'error': "OS not yet supported!"}
	
	rpmInstalled = ssh_conn.cmd_output("rpm -qa | grep %s" %HYP_CONFIG_RPM)
	if rpmInstalled == "":
		ret_code = ssh_conn.return_code("yum -y localinstall http://%s/configurators/%s.rpm" % (siteDomain,HYP_CONFIG_RPM))
		if ret_code != 0:
			ssh_conn.close()
			return {'error': "Hypervisor auto configuration failed"}
	
	if lv_sasl_login != '' and lv_sasl_passwd != '':
		ret_code = ssh_conn.return_code("echo '%s' | saslpasswd2 -p -f /etc/libvirt/passwd.db -a libvirt %s" % (lv_sasl_passwd,lv_sasl_login))
		if ret_code != 0:
			return {'error': "Could not create SASL account"}
		else:
			return {'success': "Hypervisor configuration sucessful"}
	else:
		return {'success': "Hypervisor RPM already installed"}
...
def dashboard(request):
	...
	if request.method == 'POST':
		...
		if request.POST.get('add_host', '') :
			...
			passwd_c	= request.POST.get('kvm_passwd_c', '')

			ssh_login	= request.POST.get('ssh_login', '')
			ssh_passwd	= request.POST.get('ssh_passwd', '')
			...
			if not errors:
				if ssh_login and ssh_passwd:
					if passwd_c and (passwd_c == passwd):
						result = ssh_conn_config(ipaddr, ssh_login, ssh_passwd, login, passwd)
						if result.has_key('error'):
							errors.append(result['error'])
					else:
						errors.append("SASL password mismatch!")
